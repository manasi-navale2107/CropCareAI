...
exports.signup = catchAsync(async (req, res, next) => {
  try {
    ...
    try {
      ...
      console.error("Failed to send welcome email, but continuing signup:", emailErr.message);
    }
    createSendToken(newUser, 201, res);
  } catch (error) {
    if (error.code === 11000) {
      return next(new AppError("Email or phone number already in use. Please sign in instead.", 400));
    }
    return next(new AppError(error.message || "An unexpected error occurred during signup", 500));
  }
});
exports.login = catchAsync(async (req, res, next) => {
  ...
  // 2) Check if the provided input is an email or phone number and find the user
  let user;
  ...
  // 3) If user does not exist or password is incorrect
