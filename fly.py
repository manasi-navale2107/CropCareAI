...
def home():
    return jsonify({
        "message": "CropCare AI ML API is running",
        "available_endpoints": [
            "/detect_crop_disease",
            "/detect_crop_disease_video",
            "/predict_crop",
            "/withinfo_predict_crop",
            "/predict_fertilizer",
            "/translate"
        ]
    })
...
def translate_batch(texts: Union[str, List[str]], dest: str = 'mr') -> Union[str, List[str]]:
    ...
    try:
        if isinstance(texts, str):
            translation = GoogleTranslator(source='auto', target=dest).translate(texts)
            return translation
